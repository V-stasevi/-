...
class Ghost:
    ...
    def is_dead(self):
        ...
        if self.pinky.rect.colliderect(Pacman.rect) and self.state != common:            #pacman.rect нужно нормльно привязать
            self.direction = up
            self.x = 15 * 16 - 16
            self.y = 15 * 16
            self.__update_system_position()
    ...
    #     if self.pinky.rect.colliderect(Pacman.rect) and Pacman.state == common:
    #         pass
    #         where pacman dies
    #         or
    ...
    def collosion(self):
        if self.pinky.rect.colliderect(Pacman.rect) and self.state == common:
